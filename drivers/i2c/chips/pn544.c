...
#if defined(CONFIG_MACH_A32_UL_EMEA) || defined(CONFIG_MACH_DUMMY)
#define  NFC_PWR_GPIO_CONTROL
#endif

#if defined(CONFIG_MACH_M8QL_UL)
static  struct regulator *ncp6924_ldo3;
#endif
		...
		const struct i2c_device_id *id)
{
	...
	I("%s:\n", __func__);
#if defined(CONFIG_MACH_M8QL_UL)
	ncp6924_ldo3 = regulator_get(&client->dev, "ncp6924_ldo3");
	I("%s : ncp6924_ldo3 workaround regulator_get\n", __func__);
	if (ncp6924_ldo3< 0) {
		E("%s : ncp6924_ldo3 workaround regulator_get fail\n", __func__);
		return -ENODEV;
	}
	ret = regulator_set_voltage(ncp6924_ldo3, 1800000, 1800000);
	I("%s : ncp6924_ldo3 workaround regulator_set_voltage\n", __func__);
	if (ret < 0) {
		E("%s : ncp6924_ldo3 workaround regulator_set_voltage fail\n", __func__);
		return -ENODEV;
	}
	ret = regulator_enable(ncp6924_ldo3);
	I("%s : ncp6924_ldo3 workaround regulator_enable\n", __func__);
	I("%s : ncp6924_ldo3 workaround regulator_is_enabled = %d\n", __func__, regulator_is_enabled(ncp6924_ldo3));
	if (ret < 0) {
		E("%s : ncp6924_ldo3 workaround regulator_enable fail\n", __func__);
		return -ENODEV;
	}
#endif
