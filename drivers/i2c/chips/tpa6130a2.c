...
struct tpa6130a2_data {
	...
	struct hs_config en_gpio_config;
};

struct tpa61xx_config_data tpa6130_config_data;
...
static void tpa6130a2_poweron(struct work_struct *work)
{
	...
	if (data->power_state == 0) { //off->on
		...
		if(data->en_gpio_config.init)
			gpio_set_value(data->en_gpio_config.gpio.gpio_no, 1);
#endif
		msleep(10);
...
#else
		if(data->en_gpio_config.init)
			gpio_set_value(data->en_gpio_config.gpio.gpio_no, 0);
#endif
		msleep(10);
...
int dump_amp_acoustic_table(struct tpa61xx_config_data *amp_data)
{
    int i, j;
...
//	enum AMP_GPIO_STATUS curgpiostatus;
	...
	case TPA6130_SET_PARAM:
		if (copy_from_user(&tpa6130_config_data, argp, sizeof(struct tpa61xx_config_data))) {
			pr_err("%s: copy from user failed.\n", __func__);
			return -EFAULT;
		}
...
static int tpa6130a2_dtparse_en_gpio(struct device *dev,
...
#else
	pconfig->gpio.gpio_no = of_get_named_gpio(dev->of_node, \
					pconfig->gpio.gpio_name, 0);
#endif

	if(pconfig->gpio.gpio_no < 0) {
		pr_err("%s: hs get gpio %s fail\n",__func__,pconfig->gpio.gpio_name);
		return 0;
	} else
		pr_info("%s: hs gpio %s no %d\n",__func__,pconfig->gpio.gpio_name,pconfig->gpio.gpio_no);

#ifdef CONFIG_TI_TCA6418
	ret = ioexp_gpio_get_value(pconfig->gpio.gpio_no);
	if(ret < 0) {
		pr_err("%s: hs get gpio %s status fail\n",__func__,pconfig->gpio.gpio_name);
		return ret;
	}
	pr_info("%s: hs gpio: %s status: %d\n",__func__,pconfig->gpio.gpio_name, ret);
#else
	ret = gpio_request(pconfig->gpio.gpio_no, pconfig->gpio.gpio_name);
	if (ret) {
		pr_err(	"%s: Failed to request gpio %d error %d\n",
			__func__, pconfig->gpio.gpio_no, ret);
		gpio_free(pconfig->gpio.gpio_no);
		return ret;
	}
	gpio_direction_output(pconfig->gpio.gpio_no, 0);
#endif
	pconfig->init = 1;
	return 0;
}
...
//=====================================
	...
	data->en_gpio_config.init = 0;
	data->en_gpio_config.gpio.gpio_name = "en-gpio";
	tpa6130a2_dtparse_en_gpio(dev, &data->en_gpio_config);
