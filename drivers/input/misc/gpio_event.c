...
static void clear_hw_reset(uint32_t clear_gpio)
{
	...
	msleep(100);
	ret |= gpio_direction_output(clear_gpio, 1);
				...
				struct gpio_event_platform_data *event_info)
{
	...
	ret = gpio_request(keypad_input_info.clr_gpio, "pwr_mistouch");
	KEY_LOGI("DT:clr gpio[%d] load\n", keypad_input_info.clr_gpio);

	if (ret) {
		KEY_LOGE("%s: gpio_request failed for %d\n",
			__func__, keypad_input_info.clr_gpio);
	}
	ret = gpio_direction_output(keypad_input_info.clr_gpio, 1);
	if (ret) {
		KEY_LOGE("%s: gpio_direction_input failed for %d\n",
			__func__, keypad_input_info.clr_gpio);
	}
#endif
