...
{
	uint8 SensorIndex;
	if ((Sensor==OV2722_PWDN)||(Sensor==SUB_CAM_PWDN))
	{
		SensorIndex = 0x80;
	}
	else if ((Sensor==S5K5E2_PWDN)||(Sensor==FRONT_CAM_PWDN))
	{
		SensorIndex = 0x40;
	}
	else
	...
	if (!((receive_byte_via_SPI(0x692B) & SensorIndex) && (receive_byte_via_SPI(0x6601) & SensorIndex)))
	{
		transmit_byte_via_SPI(0x692B, (receive_byte_via_SPI(0x692B)|SensorIndex));
		transmit_byte_via_SPI(0x6601, (receive_byte_via_SPI(0x6601)|SensorIndex));
	}
	
	if (Output&0x01)
	{
		transmit_byte_via_SPI(0x6611, (receive_byte_via_SPI(0x6611)|SensorIndex));
	}
	else
	{	
		transmit_byte_via_SPI(0x6611, (receive_byte_via_SPI(0x6611)&(~SensorIndex)));
