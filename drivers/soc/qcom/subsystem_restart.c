...
int subsystem_restart(const char *name)
{
	...
	/* To Prevent wlan driver trigger modem SSR but not set crash status */
	/* Force to set crash status as true while SSR for modem, MAX length=40 */
	if (!strncmp(name, "modem", 40))
	{
		dev->crashed = true;
	}
