...
struct android_dev {
	...
	wait_queue_head_t wait_close_callback_q;
};
...
static void functionfs_closed_callback(struct ffs_data *ffs)
{
	...
	pr_info("%s: wait ffs_function_enable complete\n", __func__);
#define FFS_ENABLED_TIMEOUT_MS 100
	if (0 == wait_event_interruptible_timeout(dev->wait_q,
		...
		pr_info("%s: ffs_function_enable complete\n", __func__);

	config->dev = NULL;

	pr_info("%s:set config open as false \n", __func__);
	config->opened = false;
	config->data = NULL;
...
static void customize_board_info(struct android_usb_platform_data * pdata)
{
	...
	pdata->serial_number = board_serialno();
	...
	if ((!(get_radio_flag() & BIT(17))) && (mode == MFG_MODE_MFGKERNEL || mode == MFG_MODE_MFGKERNEL_DIAG58)) {
		pdata->fserial_init_string = "tty,tty:autobot,tty:serial,tty:autobot,tty:acm";
	}
}
...
static int android_probe(struct platform_device *pdev)
{
	...
	if (pdev->dev.of_node) {
		...
		ret = of_property_read_u16(pdev->dev.of_node,
			...
			&pdata->vendor_id);
		if (ret != 0) {
			pr_info("%s(): \"htc,vendor_id\" not found(%d), use default.\n", __func__, ret);
			pdata->vendor_id = 0x0bb4;
		}
		...
		ret = of_property_read_u16(pdev->dev.of_node,
			...
			&pdata->product_id);
		if (ret != 0) {
			pr_info("%s(): \"htc,product_id\" not found(%d).\n", __func__, ret);
		}
		...
		ret = of_property_read_u32(pdev->dev.of_node,
			...
			&pdata->nluns);
		if (ret != 0) {
			pr_info("%s(): \"htc,nluns\" not found(%d).\n", __func__, ret);
		}
		...
		ret = of_property_read_u32(pdev->dev.of_node,
			...
			&pdata->cdrom_lun);
		if (ret != 0) {
			pr_info("%s(): \"htc,cdrom_lun\" not found(%d).\n", __func__, ret);
		}

		ret = of_property_read_string(pdev->dev.of_node,
			"htc,usb_rmnet_interface",
			&pdata->usb_rmnet_interface);
		if (ret != 0) {
			pr_info("%s(): \"htc,usb_rmnet_interface\" not found(%d).\n", __func__, ret);
		}
		
		of_property_read_string(pdev->dev.of_node,
			"htc,usb_diag_interface",
			&pdata->usb_diag_interface);
		if (ret != 0) {
			pr_info("%s(): \"htc,usb_diag_interface\" not found(%d).\n", __func__, ret);
		}

		of_property_read_string(pdev->dev.of_node,
			"htc,fserial_init_string",
			&pdata->fserial_init_string);
		if (ret != 0) {
			pr_info("%s(): \"htc,fserial_init_string\" not found(%d).\n", __func__, ret);
		}

		customize_board_info(pdata);
	} else {
		printk(KERN_WARNING "[USB] %s: no device tree found\n", __func__);
	...
	init_waitqueue_head(&android_dev->wait_close_callback_q);
