...
int android_switch_function(unsigned func)
{
	...
	if (product && dev->pdata) {
		pr_info("%s(): product->vendor_id=[%d], product->product_id=[%d], dev->pdata->vendor_id=[%d]\n",
			__func__, product->vendor_id, product->product_id, dev->pdata->vendor_id);
		vendor_id = product->vendor_id ? product->vendor_id : dev->pdata->vendor_id;
		product_id = product->product_id;
	} else if (dev->pdata) {
		pr_info("%s(): dev->pdata->vendor_id=[%d], dev->pdata->product_id=[%d]\n",
			__func__, dev->pdata->vendor_id, dev->pdata->product_id);
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (carry != 0)
		USB_ERR("%s: stack corruppted\n", __func__);
	if (ret < 0) {
		USB_ERR("%s: %d\n", __func__, (int)ret);
		return 0;
	}

	ret = android_switch_function((unsigned)u);
