			...
			unsigned long action, void *priv)
{
	...
	if (udev->bus != otg->host) {
		printk(KERN_WARNING "[USB]%s: in peri mode, goto out\n", __func__);
		goto out;
	} else
		printk(KERN_WARNING "[USB]%s: in host mode\n", __func__);
...
static void msm_otg_chg_check_timer_func(unsigned long data)
{
	...
	if ((readl_relaxed(USB_PORTSC) & PORTSC_LS) == PORTSC_LS ||cable_get_accessory_type() == DOCK_STATE_CAR) {
		USB_INFO( "DCP is detected as SDP\n");
...
static void msm_id_status_w(struct work_struct *w)
{
	...
	printk(KERN_WARNING "ID status_w, motg->phy.state=[%x]\n", motg->phy.state);
...
static int msm_otg_probe(struct platform_device *pdev)
{
	...
	if (1) {
		...
		psy = power_supply_get_by_name("usb");
		dev_info(motg->phy.dev, "[USB] psy=[0x%llx]\n", (unsigned long long)psy);
...
int64_t  htc_qpnp_adc_get_usbid_adc(void)
{
	...
	printk(KERN_INFO "[USB] %s : vadc_chip %llx\n",__func__,(unsigned long long)motg->vadc_chip);
