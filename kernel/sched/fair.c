...
unsigned int sysctl_sched_move_to_big = 0;


static inline int move_to_big(int cpu)
{
	if(cpu_rq(cpu)->max_possible_freq < max_possible_freq && sysctl_sched_move_to_big){
		return 1;
	}
	return 0;
}
