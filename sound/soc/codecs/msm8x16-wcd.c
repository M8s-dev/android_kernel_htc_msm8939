					      ...
					      int event, bool enable)
{
	...
	default:
		dev_err(codec->dev, "%s: Invalid micb event %d\n",
			__func__, event);
		return;
		...
		struct snd_kcontrol *kcontrol, int event)
{
	...
	if (w->shift > ON_DEMAND_MICBIAS) {
		dev_err(codec->dev, "%s: error index > Demand supplies",
			__func__);
		ret = -EINVAL;
		goto out;
	}
...
static void msm8x16_skip_imped_detect(struct snd_soc_codec *codec)
{
	...
	if(codec != NULL) {
		...
		pr_info("%s: Codec pointer is NULL\n", __func__);
	}
}
	...
	struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_POST_PMU:
		...
		if (strnstr(w->name, internal1_text, strlen(w->name))) {
#ifdef CONFIG_HTC_AUD_MIC_REG_SETTING_8916
			snd_soc_update_bits(codec, micb_int_reg, 0xff, 0x18);
			snd_soc_update_bits(codec, MSM8X16_WCD_A_ANALOG_MICB_1_EN, 0xff, 0x84);
#else
...
#ifdef CONFIG_AUD_MICB_INT_RBIAS_DISABLE
		else if (strnstr(w->name, external_text, strlen(w->name))) {
			snd_soc_update_bits(codec, MSM8X16_WCD_A_ANALOG_MICB_2_EN, 0xff, 0x00);
		}
		snd_soc_update_bits(codec, micb_int_reg, 0xff, 0x00);
#endif

#ifdef CONFIG_AUD_ALL_MIC_CHN_ODM
